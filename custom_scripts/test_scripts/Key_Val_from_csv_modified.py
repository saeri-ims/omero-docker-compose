# coding=utf-8
"""
 MIF/Add_Key_Val_from_csv_modified.py

 Adds key-value (kv) metadata to a dataset from a csv file
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    for ds in datasets:
        ...
        #commented out as i thing a dictionary is not needed simply because there is only 1 dataset with 1 ID
        #dict_name_id={}
        ...
        header =data[0]
        kv_data = header[1:]  # first header is the fimename columns but in my case is not
        rows    = data[1:]

        #nimg_updated = 0  #commented as i think it was linked to image and i don't want to deal with images
        #for row in rows: # loop over images
            #img_name = row[0]
                ...
                data_set    = conn.getObject("Dataset", ids) # get the dataset instead of getting the images

                existing_kv = get_existing_MapAnnotions( data_set )
                ...
                if( existing_kv != updated_kv ):
                    #nimg_updated = nimg_updated + 1  #commmented as refer to images
                    print("The key-values pairs are different")
                    remove_MapAnnotations( conn, 'Dataset', ds.getId()  )
    ...
    return "Added {} kv pairs to {}/{} files  ".format(len(header)-1,nimg_updated,len(dict_name_id))   #i don't know how to resolve this as there are parameters that i have commented earlier on
