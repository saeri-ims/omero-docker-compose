...
data_types = [rstring('Image')]  #added also Dataset
client = scripts.client(
    "Scripting_Service_Example_modified3.py",
    ("Example script to use as a template for getting started with the"
     " scripting service."),
    # first parameter
    scripts.String(
    "Data_Type", optional=False, grouping="1", description="The data you want to work with.", values=data_types, default="Image"),
    # second parameter
    scripts.List(
    "IDs", optional=False, grouping="2", description="List of Dataset IDs or Image IDs").ofType(rlong(0)),
)
...
with open("list_names_of_selected_images.txt", "w") as f:
    for i in images:
    	print i.name, '\n'/
        f.write(i.name)
        f.write('\n')
        print os.path.basename(i), '\n'  #added path but i am not sure that it is correct
        f.write(os.path.basename(i))
        f.write('\n')

file_ann = conn.createFileAnnfromLocalFile("list_names_of_selected_images.txt", mimetype="text/plain", ns="image.names.foo")
